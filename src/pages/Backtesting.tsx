...
export default function Backtesting() {
  ...
  const baselineByLinha = new Map<number, number | null>(
    baselines.map((b) => [b.linha, b.taxa_over]),
  );
  ...
  return (
    <div className="page-container space-y-8">
      ...
      {!ligaId && (
        <p className="text-sm text-muted-foreground">Carregando liga…</p>
      )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              {loading9 ? (
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Ainda não há dados de backtest para {ligaNome}.
                </p>
              )}
            </div>
          </div>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <span className="text-foreground font-semibold">{ligaNome}</span> — mostrando só
              os números reais abaixo.
            </p>
          </div>
        </motion.section>
      ))}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="text-2xl font-display mt-1">
                {loading5 ? '…' : over5 ? `${taxa5.toFixed(1)}%` : '—'}
              </div>
              {over5 && (
                <>
      ...
      {ligaId && (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            Dataset: {totalJogos ? `${totalJogos.toLocaleString('pt-BR')} jogos` : 'histórico'} de {ligaNome}.
            Linhas com taxa ≥ 85% ou ≤ 30% não oferecem valor prático contra as odds típicas de mercado.
          </p>
        </motion.section>
      )}
    </div>
