...
export default function Times() {
  ...
  const radarData = useMemo(() => {
    ...
    return [
      ...
      {
        stat: "Escanteios",
        A: (teamA.media_escanteios_jogo / maxEsc) * 100,
        B: (teamB.media_escanteios_jogo / maxEsc) * 100,
      },
  ...
  return (
    <div className="page-container space-y-8">
      <SEO
        title={`Times — ${ligaAtual?.nome ?? ""}`.trim()}
        description={`Classificação, forma recente, comparação H2H e estatísticas casa/fora dos clubes${ligaAtual?.nome ? ` do ${ligaAtual.nome}` : ""}.`}
        path="/times"
      />
      {/* Header */}
      <motion.div
            ...
            <div className="p-4 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold">
                  ...
                  {untilRodada != null && (
                    <span className="text-muted-foreground font-normal"> · até a rodada {untilRodada}</span>
                  )}
                </h2>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="overflow-auto">
                          ...
                          <button
                            onClick={() => {
                              if (classSortBy === key) setClassSortAsc(!classSortAsc);
                                ...
                                setClassSortBy(key as ClassSortKey);
                                setClassSortAsc(false);
                              }
                            }}
                    ...
                    return sorted.map((t, i) => {
                      return (
                        <tr key={t.team_sigla} className={getZoneStyle(i + 1)}>
                          ...
                          <td
                            className={`text-center font-mono text-sm ${t.saldo_gols > 0 ? "text-bet-green" : t.saldo_gols < 0 ? "text-destructive" : ""}`}
                          >
                            {t.saldo_gols > 0 ? "+" : ""}
                            {t.saldo_gols}
                          </td>
                          ...
                          <td
                            className={`text-center font-mono text-sm ${t.media_escanteios >= 10 ? "text-bet-green font-bold" : ""}`}
                          >
                            {t.media_escanteios.toFixed(1)}
                          </td>
                          <td>
                            <div className="flex gap-0.5">
                              {t.forma_5jogos.split("").map((c, ci) => {
                                ...
                                return (
                                  <span
                                    key={ci}
                                    className={`w-5 h-5 flex items-center justify-center rounded text-[9px] font-bold text-white ${color}`}
                                  >
                                    {c === "-" ? "" : c}
                                  </span>
                                );
                          ...
                          <td>
                            {teamIdByName.get(t.team_nome) != null && (
                              <Link
                                to={`/${ligaAtual?.espn_slug ?? ""}/times/${teamIdByName.get(t.team_nome)}`}
              ...
              <div className="flex flex-wrap gap-4 p-4 border-t border-border text-[11px] text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-blue-500" /> Libertadores
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-blue-400/60" /> Pré-Libertadores
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-orange-400/60" /> Sul-Americana
                </span>
                <span className="flex items-center gap-1.5">
          ...
          >
            ...
            <div className="mb-4">
              ...
              >
                ...
                {allTeamNames
                  .filter((t) => !evolutionTeams.includes(t))
                  .map((t) => (
              ...
              <ResponsiveContainer width="100%" height={360}>
                <LineChart data={evolutionChartData} margin={{ top: 10, right: 20, bottom: 10, left: 0 }}>
                  ...
                  <ReferenceLine
                    ...
                    strokeDasharray="4 4"
                    strokeOpacity={0.6}
                    label={{ value: "Libertadores", position: "insideTopRight", fill: "#3b82f6", fontSize: 9 }}
                  />
                  <ReferenceLine
                    y={6.5}
                    stroke="#60a5fa"
                    strokeDasharray="4 4"
                    strokeOpacity={0.5}
                    label={{ value: "Pré-Lib.", position: "insideTopRight", fill: "#60a5fa", fontSize: 9 }}
                  />
                  <ReferenceLine
                    y={12.5}
                    stroke="#fb923c"
                    strokeDasharray="4 4"
                    strokeOpacity={0.5}
                    label={{ value: "Sul-Americana", position: "insideTopRight", fill: "#fb923c", fontSize: 9 }}
                  />
                  <ReferenceLine
                    y={16.5}
                    stroke="#ef4444"
                    strokeDasharray="4 4"
                    strokeOpacity={0.6}
                    label={{ value: "Rebaixamento", position: "insideTopRight", fill: "#ef4444", fontSize: 9 }}
                  />
                    ...
                    <thead className="sticky top-0 bg-card z-10">
                      <tr>
                        ...
                        <th className="text-center">
                          <button
                            onClick={() => setSortBy("media_gols_jogo")}
                        ...
                        <th className="text-center">
                          <button
                            onClick={() => setSortBy("media_escanteios_jogo")}
                            className="inline-flex items-center gap-1 hover:text-foreground transition-colors"
                          >
                        ...
                        <th className="text-center">
                          <button
                            onClick={() => setSortBy("media_cartoes_jogo")}
                            className="inline-flex items-center gap-1 hover:text-foreground transition-colors"
                          >
                    ...
                    <tbody>
                      {sorted.map((t, i) => (
                        <tr key={t.sigla} className={i < 3 ? "bg-primary/[0.03]" : ""}>
                          ...
                          <td className="text-center font-mono text-sm text-bet-green">
                            {t.media_gols_jogo.toFixed(1)}
                          </td>
            ...
            >
              ...
              <div className="flex gap-3 mb-5">
                ...
                >
                  ...
                  {times?.map((t) => (
                    <option key={t.sigla} value={t.nome}>
                      {t.nome}
                    </option>
                  ))}
                </select>
                    ...
                    {[
                      ...
                      <div key={label} className="bg-secondary/50 rounded-lg p-3 text-center">
                        ...
                        <div className="flex items-center justify-center gap-3">
                          <span
                            className={`font-mono text-sm font-bold ${a > b ? "text-bet-green" : "text-muted-foreground"}`}
                          >
                            {a.toFixed(1)}
                          </span>
                          <span className="text-[10px] text-muted-foreground">vs</span>
                          <span
                            className={`font-mono text-sm font-bold ${b > a ? "text-[#60a5fa]" : "text-muted-foreground"}`}
                          >
                            {b.toFixed(1)}
                          </span>
                        </div>
